...
完成一次配置后，集成会定时更新绑定户号的每日用电量、本月累计电量、峰谷平尖电量，以及
最近结算月和本年累计的电量、电费；登录 Token 失效时会自动使用已保存的密码重新登录。
...
| 最近结算月电量 | kWh | 最近一个已经出账月份的结算电量，不是本月实时累计值 |
| 最近结算月电费 | CNY | 最近一个已经出账月份的结算电费 |
...
“最近一日电量”会自动采用服务端最新发布的日期，不会把当天缺失值填成 `0`。月度电费则来自
