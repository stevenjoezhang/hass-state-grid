...
| 实体名称 | 单位 | 说明 |
|---|---:|---|
| 最新日用电量 | kWh | 上游已发布的最新一条每日用电量，通常是昨天，也可能更早 |
| 本月用电量 | kWh | 上游月累计值；缺失时由每日记录相加 |
| 上一日电费 | CNY | 每日记录中向前查找到的最近一条非空电费，并附实际日期 |
...
“上一日电费”也包含 `latest_date`。该日期可能早于“最新日用电量”的日期，因为部分地区不会
每天返回费用字段；集成不会使用月度账单金额冒充日费用。
...
“最新日用电量”和“最近结算月电量”是单日或单月历史快照，不是连续累计表值，因此未设置
`state_class`，不会出现在能源面板的正常选择列表中，以防误选。

当天电量通常不会实时出现，截图中 8 月 21 日晚只能看到 8 月 20 日的数据就是正常的上游延迟。
“最新日用电量”会自动采用服务端最新发布的日期，不会把当天缺失值填成 `0`。月度电费则来自
